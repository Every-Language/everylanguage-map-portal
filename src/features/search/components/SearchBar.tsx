...
export const SearchBar: React.FC<SearchBarProps> = ({ onSelect, className }) => {
  ...
  return (
    <div ref={rootRef} className={`absolute top-4 left-1/2 -translate-x-1/2 z-20 w-full max-w-xl ${className ?? ''}`}>
      <div className="flex gap-2">
        <div className="flex-1 relative">
          <input
            ...
            className="w-full rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white/95 dark:bg-neutral-900/95 px-4 py-2 shadow focus:outline-none focus:ring-2 focus:ring-accent-600"
          />
          {isOpen && (results.length > 0 || isLoading) && (
            <div className="absolute mt-1 left-0 right-0 max-h-[50vh] overflow-auto rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 shadow-lg">
              ...
              {!isLoading && results.map((r, idx) => (
                <button
                  key={`${r.kind}-${r.id}-${idx}`}
                  onClick={() => { setIsOpen(false); onSelect(r) }}
